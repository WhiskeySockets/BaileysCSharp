...
namespace WhatsSocket.Core
{
    public partial class BaseSocket
    {
        ...
        {
            return await SocketHelper.ProcessNodeWithBuffer(node, "handling ack", HandleAck);
        }
        ...
        private async Task HandleMessage(BinaryNode node)
        {
            ...
            else
            {
                ...
                var isAnyHistoryMsg = History.GetHistoryMsg(result.Msg.Message);
            ...
            UpsertMessage(result.Msg, node.getattr("offline") != null ? "append" : "notify");
